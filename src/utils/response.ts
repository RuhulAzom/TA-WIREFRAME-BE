...
export const getZodErrorMessage = (issues: any[]) => {
  ...
  issues.forEach((issue) => {
    ...
    // });
    // message = `${message}\n\nExpected: ${expected}\nReceived: ${received}`;
    message = `Data ${path.join(".")} tidak valid!`;
    if (msg === "Required") {
      message = `Data ${path.join(".")} dibutuhkan!`;
    } else if (code === "invalid_type") {
      message = `Data ${path.join(".")} seharusnya ${expected}`;
    }
    if (code === "too_small") {
      message = `Data ${path.join(".")} minimal ${issue.minimum} characters`;
    }
    if (code === "too_big") {
      message = `Data ${path.join(".")} maksimal ${issue.maximum} characters`;
    }
    if (code === "custom") {
      message = msg;
    }
